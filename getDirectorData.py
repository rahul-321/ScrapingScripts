...
query="select * from %s "%(conf['DirectorTable'])
directorIdList=GenericDAL.QueryResult(query,conf)

#director="name/nm0000095/?ref_=tt_ov_dr"
for director in directorIdList:
	if float(director[2])!=0.00:
		continue
	start_page1= requests.get("http://www.imdb.com/"+director[0]+"#director")
	tree1=html.fromstring(start_page1.text)

	movies=tree1.xpath('//div[@class="filmo-category-section"]/div[starts-with(@id,"director")]/b/a/@href')
	dates=tree1.xpath('//div[@class="filmo-category-section"]/div[starts-with(@id,"director")]/span[@class="year_column"]/text()')
	...
	try:
		...
		query="update %s set score= %s where id= '%s'"%(conf['DirectorTable'],score,director[0])
	except:
		continue
